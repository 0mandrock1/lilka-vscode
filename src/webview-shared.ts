...
export function getWebviewHtml(webview: vscode.Webview, extensionUri: vscode.Uri): string {
  ...
  const uiUri      = webview.asWebviewUri(vscode.Uri.joinPath(extensionUri, 'media', 'webview.js'));
...
<meta http-equiv="Content-Security-Policy"
  content="default-src 'none'; script-src ${cspSrc}; style-src 'unsafe-inline';">
<style>
...
.tab-btn{flex:1;padding:5px 2px;font-size:10px;font-family:inherit;
...
select,input{background:var(--vscode-input-background);color:var(--vscode-input-foreground);
  border:1px solid var(--vscode-input-border,#555);border-radius:2px;
  padding:2px 5px;font-size:11px;font-family:var(--vscode-font-family)}
input[type=number]{width:64px}
input.wide{flex:1;min-width:80px}
.btn{background:var(--vscode-button-background);color:var(--vscode-button-foreground);
  border:none;border-radius:2px;padding:3px 8px;font-size:11px;cursor:pointer;white-space:nowrap}
...
.msg{padding:6px 8px;border-radius:3px;font-size:11px;line-height:1.55;word-break:break-word}
.msg.user{background:var(--vscode-inputOption-activeBackground);align-self:flex-end;max-width:90%}
.msg.bot{background:var(--vscode-editor-inactiveSelectionBackground);align-self:flex-start;max-width:100%}
.msg.thinking{color:var(--vscode-descriptionForeground);font-style:italic}
.msg.bot code{font-family:var(--vscode-editor-font-family,monospace);
  background:var(--vscode-textCodeBlock-background);padding:0 3px;border-radius:2px;font-size:10px}
.msg.bot pre{background:var(--vscode-textCodeBlock-background);border-radius:3px;padding:6px;
  overflow-x:auto;margin:4px 0;font-family:var(--vscode-editor-font-family,monospace);
  font-size:10px;white-space:pre}
#input-row{display:flex;gap:5px;padding:6px 10px;
  border-top:1px solid var(--vscode-panel-border);flex-shrink:0}
#q{flex:1;padding:4px 7px;background:var(--vscode-input-background);
...
#screen{display:block;border:1px solid var(--vscode-panel-border);
  image-rendering:pixelated;image-rendering:crisp-edges;cursor:pointer;max-width:100%;outline:none}
...
.emu-btn{width:28px;height:28px;background:var(--vscode-button-secondaryBackground);
  color:var(--vscode-button-secondaryForeground);border:1px solid var(--vscode-panel-border);
  border-radius:3px;font-size:11px;cursor:pointer;user-select:none;touch-action:none;
  display:flex;align-items:center;justify-content:center;padding:0}
.emu-btn:active{background:var(--vscode-button-background);color:var(--vscode-button-foreground)}
.abcd-grid{display:grid;grid-template-columns:1fr 1fr;gap:2px}
.sys-btns{display:flex;gap:4px;padding:2px 0}
.sys-btns .emu-btn{width:40px}
.emu-toolbar{display:flex;gap:5px;padding:4px 8px;flex-wrap:wrap;justify-content:center}
#emu-log{width:100%;max-height:110px;overflow-y:auto;
...
<div class="tabs">
  <button class="tab-btn active" data-tab="conn">З'єднання</button>
  <button class="tab-btn"        data-tab="docs">Docs</button>
  <button class="tab-btn"        data-tab="emu">Емулятор</button>
</div>
...
<div id="tab-conn" class="tab-content active">
  <div class="row">
    ...
    <button class="btn sec" id="btn-refresh-ports" title="Оновити">&#8635;</button>
  </div>
  <div class="row"><label>Baud</label><input type="number" id="baud" value="115200"></div>
  <div class="row">
  ...
  <div class="row">
    <label>Model</label>
    <input type="text" id="ai-model" class="wide" placeholder="llama3.2:3b">
  </div>
...
<div id="tab-docs" class="tab-content">
  <div id="messages">
    <div class="msg bot">Питай про Lilka API.<br>
    Напр: <em>draw_line</em>, <em>colors.white</em>, <em>adc</em>, <em>tcp</em></div>
  </div>
  <div id="input-row">
    <input id="q" type="text" placeholder="display / buzzer / tcp / gpio…">
...
<div id="tab-emu" class="tab-content">
  ...
  <p class="hint">WASD/стрілки Z=A X=B C D Enter=Start — клікни канвас для фокусу</p>
  ...
  <div class="emu-toolbar">
    ...
    <button class="btn sec" id="btn-stop">&#9632; Stop</button>
    <button class="btn sec" id="btn-clearlog">&#10005; Log</button>
  </div>
  <div id="emu-log"></div>
</div>
